...
Deploy automation already downloaded and cached an openSUSE Leap 16.0 Cloud image on the KVM host, serving it at `http://192.168.122.1:8889/` so this step needs no internet access from inside the cluster. This is the `-Cloud` build variant specifically because it ships cloud-init, which the next step depends on — the plain `-kvm-and-xen` build of the same release is combustion-only and silently ignores cloud-init user-data.
...
| Name | `sles16` |
| URL | `http://192.168.122.1:8889/Leap-16.0-Minimal-VM.x86_64-Cloud.qcow2` |
...
Minimal user-data (installs and starts `qemu-guest-agent`; SSH access itself uses the image's own default account, `sles` for the cached Leap 16.0 Cloud image from 2.7):
